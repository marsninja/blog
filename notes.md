...
* if "run-dot" is added to the class list, the codeblock will have an additional "graph" button to allow users to see the graph attached to root in the doc. That's based on the "jac dot [filename.jac] --to_screen" command. These should be added to example intended to have an OSP graph.
...
* You should always check any jac code examples created with `jac run` or `jac dot` to validate the output is as expected.
